"""SRUNetHeavy — source from github.com/aapetukhov/efml-unet."""
...
class MBResBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.project(self.dw_conv(self.expand(x))) + self.skip(x)
...
def load_heavy_model(checkpoint_path: str) -> SRUNetHeavy:
    ...
    model = SRUNetHeavy(
        ...
        se_bottleneck_ratio=cfg.get("se_bottleneck_ratio", 4),
