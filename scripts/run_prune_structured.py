#!/usr/bin/env python3
"""Structural expand-channel pruning of SRUNetHeavy + finetune + benchmark.

Physically removes the least-important mid_ch channels from every MBResBlock,
making tensors smaller and reducing FLOPs. Gives real latency speedup on any GPU.

Usage:
  python scripts/run_prune_structured.py
  python scripts/run_prune_structured.py prune_ratio=0.25 results_name=struct_25
  python scripts/run_prune_structured.py prune_ratio=0.5 finetune.epochs=10 results_name=struct_50
  python scripts/run_prune_structured.py finetune.enabled=false results_name=struct_25_no_ft
"""
...
@hydra.main(version_base=None, config_path="../src/configs", config_name="prune_structured")
def main(cfg: DictConfig) -> None:
    ...
    baseline_params = _count_params(model)
    log.info(f"Baseline | SSIM: {baseline_ssim:.4f} | Latency: {baseline_latency:.2f} ms | Params: {baseline_params:.1f}M")

    # Structural pruning
    log.info(f"Pruning expand channels (prune_ratio={cfg.prune_ratio})...")
    block_info = prune_all_mbresblocks(model, cfg.prune_ratio, cfg.model.num_groups)
    for name, new_mid in block_info.items():
        log.info(f"  {name}: mid_ch → {new_mid}")
    result_params = _count_params(model)
    log.info(f"After pruning: {result_params:.1f}M params ({result_params/baseline_params*100:.0f}% of original)")
    ...
    results = {
        "method": "structural_expand_prune",
        "timestamp": datetime.now().isoformat(),
        "checkpoint_in": cfg.checkpoint_in,
        "checkpoint_out": cfg.checkpoint_out,
        "baseline": {
            "ssim": round(baseline_ssim, 4),
            "latency_ms": round(baseline_latency, 2),
            "params_M": round(baseline_params, 2),
        },
        "result": {
            "ssim": round(result_ssim, 4),
            "latency_ms": round(result_latency, 2),
            "params_M": round(result_params, 2),
        },
        "speedup":    round(baseline_latency / result_latency, 3),
        "ssim_drop":  round(baseline_ssim - result_ssim, 4),
        "param_ratio": round(result_params / baseline_params, 3),
